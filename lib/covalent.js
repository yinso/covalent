...
var ___SRC_BINDING___ = (function(module) {
  ...
  TextBinding = (function() {
    ...
    TextBinding.prototype.uponRefresh = function(err, res) {
      if (!err) {
        if (typeof window !== 'undefined' && this.element instanceof window.HTMLTitleElement) {
  ...
  WidgetFactory = (function() {
    ...
    function WidgetFactory(context, element, prop, depends, runtime, callback) {
      ...
      if (!this.constructor.widgets.hasOwnProperty(this.prop)) {
        throw new Error("unknown_widget_type: " + this.prop);
      }
    }
...
var ___SRC_TEMPLATE___ = (function(module) {
  ...
  Template = (function() {
    ...
    Template.make = function(template, runtime, noClone) {
      ...
      view = new this(element, runtime, noClone);
      return view;
    };
  ...
  UIView = (function() {
    ...
    UIView.prototype.initialize = function() {
      ...
      boundElements = this.$(view).filter('[data-bind]').add('[data-bind]', view).not('[covalent-inner]').toArray();
      this.$('[covalent-inner]', view).removeAttr('covalent-inner');
      if (boundElements.length !== this.template.bindingFactories.length) {
        throw new Error("Template.ctor:mismatch_bindings_length: " + boundElements.length + " != " + this.template.bindingFactories.length);
      }
...
var ___SRC_OBJECT___ = (function(module) {
  (function() {
  var EventEmitter, ObjectProxy, ProxyMap, ProxyPath,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __slice = [].slice;
  ...
  ProxyPath = (function(_super) {

    __extends(ProxyPath, _super);

    function ProxyPath(inner, prefix) {
      this.inner = inner;
      this.prefix = prefix;
      this.setMaxListeners(0);
    }

    ProxyPath.prototype._normalizePath = function(path) {
      if (this.prefix === '') {
        return path;
      } else if (path === '.') {
        return this.prefix;
  ...
  ProxyMap = (function() {
    ...
    ProxyMap.prototype.normalizePath = function(path) {
      var key, val, _ref;
      _ref = this.aliases;
      for (key in _ref) {
        val = _ref[key];
    ...
    ProxyMap.prototype.addProxy = function(path) {
      var normalized, proxy;
      normalized = this.normalizePath(path);
      proxy = new ProxyPath(this.root, normalized);
      this.proxies[path] = proxy;
      return proxy;
    };

    ProxyMap.prototype.setAlias = function(path, path2) {
      var data, keysHelper, oldData, proxy;
      keysHelper = function(oldData, newData) {
        var key, result, val;
        result = {};
        for (key in oldData) {
          val = oldData[key];
      ...
      oldData = this.root.get(path);
      data = this.root.get(path2);
      this.root.emit('set', {
        type: 'set',
        path: path,
        oldVal: oldData,
      ...
      proxy.prefix = path2;
      this.aliases[path] = path2;
      this._recurSetAlias(proxy, path, path2, keysHelper(oldData, data));
      return proxy;
    };

    ProxyMap.prototype._recurSetAlias = function(proxy, path, path2, oldData) {
      var key, newPath, oldPath, val, _results;
      if (oldData instanceof Object) {
        ...
        for (key in oldData) {
          ...
          oldPath = "" + path + "." + key;
          newPath = "" + path2 + "." + key;
          proxy = this.hasProxy(oldPath);
          if (proxy) {
            proxy.prefix = newPath;
            _results.push(this._recurSetAlias(oldPath, newPath, val));
          } else {
    ...
    ProxyMap.prototype._callSet = function(evt) {
      var key, proxy, val, _ref, _results;
      proxy = this.hasProxy(evt.path);
      this._recurseSet(proxy, evt);
      _ref = this.aliases;
      _results = [];
      for (key in _ref) {
        val = _ref[key];
        if (evt.path.indexOf(val) === 0) {
          _results.push(this._callSet({
            type: evt.type,
            path: "" + key + (evt.path.substring(val.length)),
            oldVal: evt.oldVal,
            newVal: evt.newVal
          }));
        } else {
    ...
    ProxyMap.prototype._recurseSet = function(proxy, evt) {
      ...
      } else if (!(oldVal instanceof Object) && (newVal instanceof Object)) {
        this._recurseSetInner(proxy, {
          type: 'set',
          path: path,
          oldVal: {},
    ...
    ProxyMap.prototype._callDelete = function(evt) {
      var key, proxy, val, _ref, _results;
      proxy = this.hasProxy(evt.path);
      this._recurseDelete(proxy, evt);
      _ref = this.aliases;
      _results = [];
      for (key in _ref) {
        val = _ref[key];
        if (evt.path.indexOf(val) === 0) {
          _results.push(this._callDelete({
            type: evt.type,
            path: "" + key + (evt.path.substring(val.length)),
            oldVal: evt.oldVal
          }));
    ...
    ProxyMap.prototype._callSplice = function(evt) {
      ...
      for (key in _ref) {
        val = _ref[key];
        if (evt.path.indexOf(val) === 0) {
          _results.push(this._callSplice({
            type: evt.type,
            path: "" + key + (evt.path.substring(val.length)),
            index: evt.index,
            removed: evt.removed,
    ...
    ProxyMap.prototype._callMove = function(evt) {
      var key, newPath, newToPath, proxy, toProxy, val, _ref, _results;
      proxy = this.hasProxy(evt.path);
      if (proxy) {
      ...
      for (key in _ref) {
        ...
        if (evt.path.indexOf(val) === 0) {
          ...
          toProxy = this.getProxy(newToPath);
          _results.push(this._callMove({
            type: evt.type,
            path: newPath,
            toPath: newToPath,
            toProxy: toProxy
  ...
  ObjectProxy = (function(_super) {
    ...
    ObjectProxy.prototype.get = function(path, normalized) {
      if (normalized == null) {
        normalized = this.map.normalizePath(path);
      }
      return this._get(this._path(normalized), this.data);
    };
    ...
    ObjectProxy.prototype.set = function(path, val) {
      var current, key, normalized, segs, _ref;
      normalized = this.map.normalizePath(path);
      _ref = this._pathWithKey(normalized), segs = _ref[0], key = _ref[1];
      current = this._get(segs, this.data);
      if (current instanceof Object) {
        return this._set(normalized, current, key, val);
      } else {
    ...
    ObjectProxy.prototype.splice = function(path, index, removeCount, inserted) {
      var ary, normalized;
      normalized = this.map.normalizePath(path);
      ary = this.get(path, normalized);
      if (!(ary instanceof Array)) {
        throw new Error("ObjectProxy.splice:not_an_array: " + path);
      }
      return this._splice(ary, normalized, index, removeCount, inserted);
    };
    ...
    ObjectProxy.prototype.push = function(path, inserted, evt) {
      ...
      normalized = this.map.normalizePath(path);
      ary = this.get(path, normalized);
      return this._splice(ary, normalized, ary.length, 0, inserted, evt);
    };
    ...
    ObjectProxy.prototype.pop = function(path, evt) {
      ...
      normalized = this.map.normalizePath(path);
      ary = this.get(path, normalized);
      return this._splice(ary, normalized, ary.length - 1, 1, [], evt);
    };
    ...
    ObjectProxy.prototype["delete"] = function(path) {
      var key, normalized, parent, segs, _ref;
      normalized = this.map.normalizePath(path);
      _ref = this._pathWithKey(normalized), segs = _ref[0], key = _ref[1];
      parent = this._get(segs, this.data);
      if (parent instanceof Object) {
        return this._delete(normalized, parent, key);
      } else {
...
// src/kvs
var ___SRC_KVS___ = (function(module) {
...
// src/route
var ___SRC_ROUTE___ = (function(module) {
  



(function() {
  var App, EventEmitter, Route, RouteParameter, RouteTable, isRelative, kvs, pathTo, url;

  RouteParameter = (function() {

    function RouteParameter(param) {
      this.name = param.substring(1);
      if (this.name.match(/\?$/)) {
        this.optional = true;
        this.name.replace(/\?$/, '');
      } else {
    ...
    RouteParameter.prototype.compare = function(param) {
      if (!this.optional && param.optional) {
        return -1;
      }
      if (this.optional && !param.optional) {
        return 1;
  ...
  Route = (function() {
    ...
    function Route(url) {
      this.url = url;
      this.makeRouteParams(this.url.split('/'));
    }

    Route.prototype.makeRouteParams = function(segments) {
      var i, names, param, seg, _i, _len;
      this.params = [];
      this.splat = void 0;
      ...
      for (i = _i = 0, _len = segments.length; _i < _len; i = ++_i) {
        ...
        if (seg.match(/^\:/)) {
          ...
          if (this.splat && param.splat) {
            throw new Error("multiple_splat_in_url: " + this.url);
          } else if (param.splat) {
            this.splat = param;
          } else {
            if (param.optional) {
      ...
      return this.requiredCount = this.params.length - this.optionalCount;
    };

    Route.prototype.normalizeMatchParams = function(length) {
      var i, optionalCount, output, seg, _i, _len, _ref;
      ...
      } else if (length === this.params.length) {
        return this.params;
      } else if (length < this.requiredCount) {
        return false;
      } else {
        output = [];
        i = 0;
        optionalCount = length - this.requiredCount;
        _ref = this.params;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          seg = _ref[_i];
    ...
    Route.prototype.match = function(url) {
      var i, normalized, params, seg, segments, spec, splat;
      segments = url.split('/');
      normalized = this.normalizeMatchParams(segments.length);
      ...
      if (this.splat) {
        ...
        params[this.splat.name] = splat.join('/');
      }
      return params;
    };

    Route.prototype.paramType = function(param) {
    ...
    Route.prototype.compare = function(route) {
      var result;
      result = this._compare(route);
      if (result !== 0) {
        return result;
      }
      if (!this.splat && route.splat) {
        return -1;
      }
      if (this.splat && !route.splat) {
        return 1;
    ...
    Route.prototype._compare = function(route) {
      var i, lhs, lhsType, result, rhs, rhsType, _i, _ref;
      if (this.params.length < route.params.length) {
        return -1;
      }
      if (this.params.length > route.params.length) {
        return 1;
      ...
      for (i = _i = 0, _ref = this.params.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        lhs = this.params[i];
        rhs = route.params[i];
        lhsType = this.paramType(lhs);
        rhsType = this.paramType(rhs);
        if (lhsType !== rhsType) {
          if (lhsType === 'string') {
            return -1;
          } else {
  ...
  RouteTable = (function() {
    ...
    RouteTable.prototype.add = function(url, callback) {
      this.routes.push([new Route(url), callback]);
      return this.routes.sort(function(a, b) {
        return a[0].compare(b[0]);
      });
    };

    RouteTable.prototype.matchID = function(url) {
      var callback, i, params, route, _i, _len, _ref, _ref1;
      _ref = this.routes;
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        _ref1 = _ref[i], route = _ref1[0], callback = _ref1[1];
    ...
    RouteTable.prototype.runID = function(_arg, cb) {
      var callback, id, params, route, url, _ref;
      url = _arg.url, params = _arg.params, id = _arg.id;
      route = this.routes[id];
      if (!route) {
        throw new Error("unknown_route: " + url);
      }
      _ref = route, route = _ref[0], callback = _ref[1];
      return cb(null, params, callback);
    };

    RouteTable.prototype.match = function(url, cb) {
      var callback, params, route, _i, _len, _ref, _ref1;
      _ref = this.routes;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        _ref1 = _ref[_i], route = _ref1[0], callback = _ref1[1];
        params = route.match(url);
        if (params !== false) {
          return cb(null, params, callback);
        }
      }
      return cb(new Error("unknown_route: " + url));
    };

    RouteTable.prototype.get = function(url) {
      var callback, route, _i, _len, _ref, _ref1;
      _ref = this.routes;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        _ref1 = _ref[_i], route = _ref1[0], callback = _ref1[1];
        if (route.url === url) {
  ...
  pathTo = function(from, to) {
    var fromPaths, helper, toPaths;
    helper = function(toPaths, fromPaths) {
      if (toPaths.length === 0) {
        return fromPaths.reverse().join('/');
      } else if (toPaths[0] === '.') {
        toPaths.shift();
        return helper(toPaths, fromPaths);
      } else if (toPaths[0] === '..') {
        if (fromPaths.length < 2) {
  ...
  App = (function() {
    ...
    function App(runtime) {
      this.runtime = runtime;
      this.$ = this.runtime.$;
      this.getRoutes = new RouteTable();
      this.postRoutes = new RouteTable();
      this.errorHandlers = {};
    }

    App.prototype.initialize = function() {
      this.setupAddressRoute();
      return this.setupAnchor();
    };

    App.prototype.setupAddressRoute = function() {
      var app, _ref,
        _this = this;
      app = this;
      return (_ref = this.$.address) != null ? _ref.change(function(evt) {
        var req;
        console.log('Address.change', evt.value);
        req = _this.findRoute('get', evt.value);
        return app.runRoute(req);
      }) : void 0;
    };

    App.prototype.setupAnchor = function() {
      var app, document,
        _this = this;
      app = this;
      document = this.$('body')[0].ownerDocument;
      console.log('@setupAnchor', document);
      this.$(document).on('click', 'a', function(evt) {
        var normalizedURL, popup;
        evt.preventDefault();
        console.log('App.a.clickMe', _this.host, location.host, _this.href, _this.href.match(/^javascript/));
        if (_this.href === '' || _this.href.match(/^javascript/)) {
          console.log('App.a.JS', _this.host, location.host, _this.href);
          return false;
        }
        if (_this.host === location.host) {
          normalizedURL = app.normalizeURL(_this.$(_this).attr('href'));
          _this.$.address.value(normalizedURL.pathname + normalizedURL.search);
        } else {
          popup = window.open(_this.href);
        }
        return false;
      });
      return this.$(document).on('submit', 'form', function(evt) {
        var form, formTarget, req;
        evt.preventDefault();
        form = _this;
        console.log('jquery.on form', form);
        formTarget = _this.$(form).attr('action');
    ...
    App.prototype.normalizeURL = function(source) {
      var parsed;
      ...
      if (parsed.host !== location.host) {
        ...
        if (isRelative(source)) {
          return url.parse(pathTo(location.hash.substring(1), source));
        } else {
          return parsed;
        }
      }
    ...
    App.prototype.get = function(url, callback) {
      return this.getRoutes.add(url, callback);
    };

    App.prototype.post = function(url, callback) {
      return this.postRoutes.add(url, callback);
    };

    App.prototype.findRoute = function(method, url) {
      var req;
      method = (method || 'get').toLowerCase();
      req = method === 'get' ? this.getRoutes.matchID(url) : this.postRoutes.matchID(url);
      req.method = method;
      return req;
    };

    App.prototype.runRoute = function(req, cb) {
      var _this = this;
      cb = function(err, params, callback) {
        if (err) {
          return cb(err);
        } else {
          return callback(req);
      ...
      if (req.method === 'get') {
        return this.getRoutes.runID(req, cb);
      } else {
        return this.postRoutes.runID(req, cb);
      }
    };

    App.prototype.redirect = function(url) {
      console.log('App.redirect', url);
      return this.$.address.value(url);
    };
    ...
    App.prototype.getJSON = function(url, cb) {
      ...
      return this.$.ajax({
        ...
        dataType: 'json',
        error: this.makeErrorHandler(cb),
        success: this.makeSuccessHandler(cb)
      });
    };

    App.prototype.postJSON = function(url, data, cb) {
      var dataString;
    ...
    App.prototype.makeErrorHandler = function(cb) {
      var _this = this;
      return function(xhr, status, err) {
        ...
        if (_this.predefinedError(error)) {
          return _this.predefinedError(error);
        } else {
    ...
    App.prototype.setErrorHandler = function(status, handler) {
      if (!this.errorHandlers.hasOwnProperty(status)) {
        return this.errorHandlers[status] = handler;
      } else {
        return console.error("App.setErrorHandler", "duplicate_error_handler", status, handler);
      }
    };

    App.prototype.predefinedError = function(err) {
      if (typeof err instanceof Error) {
        return this.errorHandlers(err.status);
      } else {
        return void 0;
      }
    };

    App.prototype.makeSuccessHandler = function(cb) {
      return function(data, status, xhr) {
        return cb(null, data);
      };
    };

    App.prototype.submit = function(_arg) {
      var dataType, error, form, success;
      form = _arg.form, error = _arg.error, dataType = _arg.dataType, success = _arg.success;
    ...
    App.prototype.postForm = function(form, cb) {
      return this.submit({
        form: form,
        error: this.makeErrorHandler(cb),
        dataType: 'json',
...
var ___SRC_RUNTIME___ = (function(module) {
  (function() {
  var Compiler, ObjectProxy, Route, Runtime, TemplateManager, WidgetFactory, builtIn;
  ...
  Runtime = (function() {
    ...
    Runtime.prototype.get = function(path) {
      return this.context.get(path);
    };

    Runtime.prototype.set = function(path, val) {
      return this.context.set(path, val);
    };

    Runtime.prototype.push = function(path, args) {
      return this.context.push(path, args);
    };

    Runtime.prototype.pop = function(path) {
      return this.context.pop(path);
    };

    Runtime.prototype.shift = function(path) {
      return this.context.shift(path);
    };

    Runtime.prototype.unshift = function(path, args) {
      return this.context.unshift(path, args);
    };

    Runtime.prototype.splice = function(path, index, removeCount, inserted) {
      return this.context.splice(path, index, removeCount, inserted);
    };

    Runtime.prototype.routeGet = function(path, callback) {
      return this.app.get(path, callback);
    };

    Runtime.prototype.routePOST = function(path, callback) {
      return this.app.post(path, callback);
    };

    Runtime.prototype.getJSON = function(url, data, cb) {
      return this.app.getJSON(url, data, cb);
    };

    Runtime.prototype.postJSON = function(url, data, cb) {
      return this.app.postJSON(url, data, cb);
    };
